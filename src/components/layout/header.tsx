    ...
    <>
      ...
      <header
        ...
        <div className="container grid grid-cols-3 h-14 items-center">
            {/* Left: Services / Work / Pricing (desktop only) */}
            <div className="flex items-center">
                <nav className="hidden md:flex items-center gap-2">
                    {navLinks.filter(l => ['/services','/portfolio','/pricing'].includes(l.href)).map(link => (
            ...
            <div className="flex justify-center">
                                <Link href="/" onClick={(e) => handleLinkClick('/', e)} className="flex items-center">
                                        {mounted && (
                                            <Image src={theme === 'dark' ? '/zero-theorem-white.svg' : '/zero-theorem-black.svg'} alt="Zero Theorem Logo" width={64} height={64} className="h-12 w-auto" />
                                        )}
                                </Link>
            </div>

            {/* Right: other links, theme toggle, contact, and mobile menu */}
